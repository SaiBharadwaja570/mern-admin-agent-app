  ...
  try {
    ...
    tasks = tasks
      .map((item) => ({
        ...
        phone: (item.Phone || item.phone || item["Phone Number"] || "").trim(),
    ...
    if (tasks.length === 0) {
      return res
        .status(400)
        .json({ message: "No valid tasks found after processing" });
    }

    const agents = await Agent.find({ adminId: req.adminId });
    ...
    while (taskIndex < tasks.length) {
      for (let a = 0; a < numAgents && taskIndex < tasks.length; a++) {
        distributed[a].push(tasks[taskIndex]);
        taskIndex++;
      }
    }
    ...
    for (let i = 0; i < numAgents; i++) {
      ...
      if (agentTasks.length > 0) {
        const createdTasks = await Task.insertMany(
          agentTasks.map((task) => ({
            agentId: agent._id,
            adminId: req.adminId, // ✅ store admin who uploaded
            ...task,
          }))
        );
